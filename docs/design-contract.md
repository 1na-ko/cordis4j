...
| D21 | Annotation injection | @Inject(qualifier) fields of an instance, assembled by Injects.injectFields(ctx, instance) into one D11 declaration: activation populates the fields as snapshots, withdrawal/retirement clears them, re-satisfaction refills; assembly fails fast on static/final/primitive fields, and an instance without annotated fields is a no-op. The compile-time processor generates the same declaration with zero-reflection accessors covering the whole superclass hierarchy; its declared boundary vs the runtime form: reachable inherited fields (public anywhere, non-private same-package) wire, unreachable ones are compile errors located on the field - reflection reaches private superclass fields, direct assignment cannot | Paper Section 6.4 sanctions annotation-mediated access via runtime reflection where the language has no transparent interception primitive; keeps core zero-dependency (JDK reflection only) |
